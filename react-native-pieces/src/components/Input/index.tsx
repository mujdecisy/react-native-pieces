...
import InputDate, { InputDateModalContent, InputDateRightButton } from './InputDate';
	...
	switch (true) {
		case props.type === InputTypes.TEXT:
			customInput = (
				<InputText
					style={styles.input}
					handleChange={value => { props.handleChange(value, ValidStates.UNDEFINED) }}
					value={props.value}
		...
		case props.type === InputTypes.NUMBER:
			customInput = (
				<InputNumber 
					style={styles.input}
					handleChange={(value, valid)=> {props.handleChange(value, valid)}}
					value={props.value}
					placeholder={props.placeholder || DEFAULT_PLACEHOLDERS[props.type]}/>
			);
			break;
		...
		case props.type === InputTypes.SINGLE || props.type === InputTypes.MULTI:
			customInput = (
				<InputSelect
					handleModalOpen={()=>{setModalVisible(true)}}
