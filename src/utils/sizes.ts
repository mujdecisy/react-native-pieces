...
export default class SizeScheme {
    ...
    private static controlScheme(scheme: ISizeScheme) {
        ...
        if (JSON.stringify(sortedOrder) !== JSON.stringify(expectedOrder) || f < 1) {
            throw new Error('Font size must be desc order and must be at least 1.');
        }
        
        if (scheme.screen.width.max < scheme.screen.width.min || scheme.screen.width.min < 1) {
            throw new Error('Max width must be greater or equals to min and all must be at least 1');
        }

        if (scheme.gap.pad < 0 || scheme.gap.pad > scheme.screen.width.min*0.4) {
            throw new Error('Pad can not be negative or can not be greater than 40% of min width.');
        }
    }
}