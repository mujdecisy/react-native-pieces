...
const InputSelect = (props: InputSelectProps) => {
  return (
    <Pressable style={props.style} onPress={props.handleModalOpen}>
      {!props.value || props.value.length < 1 ? (
        <Text
          style={{ ...props.textStyle, color: ColorScheme.get().textLight }}
        >
          {props.placeholder}
        </Text>
      ) : (
        <Text style={props.textStyle}>
          {props.value.map((e) => props.options[parseInt(e)]).join(', ')}
        </Text>
      )}
    </Pressable>
  );
