...
const COLOR_SECONDARY_LIGHT = ColorScheme.hyalo(ColorScheme.get().secondary, 0.3);
...
const Calendar = (props: CalendarProps) => {
    ...
    useEffect(() => {
        const tdate = props.targetDate || new Date();
    ...
    return (
        <View style={{marginBottom: (6-dates.length)*40}}>
            <View style={styles.monthControlCont}>
                ...
                <Text style={{ fontWeight: 'bold' }}>{`${targetDate.toLocaleString('en-us', { month: 'long' })} ${targetDate.getFullYear()}`}</Text>
                <ButtonIcon
                    faIcon={faAngleRight}
                    handleClick={() => {
                        const tdate = new Date(targetDate);
                ...
                <View style={{ ...styles.gridRow, ...{ borderTopWidth: 0 } }}>
                    {
                        ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(e => (
                            <View style={{ ...styles.gridCell, ...styles.gridHeader }} key={`tag-${e}`}>
...
export const dateToString = (date: Date) => `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date.getDate().toString().padStart(2, '0')}`;
